...
public:
  MOSInstructionSelector(const MOSTargetMachine &TM,
                             MOSSubtarget &STI,
...
MOSInstructionSelector::MOSInstructionSelector(
    const MOSTargetMachine &TM, MOSSubtarget &STI,
    MOSRegisterBankInfo &RBI)
...
bool MOSInstructionSelector::selectLshO(MachineInstr &MI) {
  assert(MI.getOpcode() == MOS::G_LSHO);
...
bool MOSInstructionSelector::selectLshE(MachineInstr &MI) {
  assert(MI.getOpcode() == MOS::G_LSHE);
...
bool MOSInstructionSelector::selectPtrAdd(MachineInstr &MI) {
  ...
  auto AddHi = Builder.buildInstr(MOS::ADCimm, {s8, s1},
...
bool MOSInstructionSelector::selectUnMergeValues(MachineInstr &MI) {
  ...
  auto LoCopy = Builder.buildInstr(MOS::COPY)
                    .addDef(Lo)
                    .addUse(Src, 0, MOS::sublo);
  constrainGenericOp(*LoCopy);
  auto HiCopy = Builder.buildInstr(MOS::COPY)
                    .addDef(Hi)
                    .addUse(Src, 0, MOS::subhi);
  constrainGenericOp(*HiCopy);
  MI.eraseFromParent();
  return true;
}

void MOSInstructionSelector::buildCopy(MachineIRBuilder &Builder,
                                           Register Dst, Register Src) {
  auto Copy = Builder.buildCopy(Dst, Src);
  constrainGenericOp(*Copy);
}

void MOSInstructionSelector::composePtr(MachineIRBuilder &Builder,
...
InstructionSelector *
llvm::createMOSInstructionSelector(const MOSTargetMachine &TM,
                                       MOSSubtarget &STI,
                                       MOSRegisterBankInfo &RBI) {
