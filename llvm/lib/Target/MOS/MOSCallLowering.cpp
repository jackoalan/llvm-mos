...
struct MOSOutgoingValueHandler : CallLowering::OutgoingValueHandler {
  ...
  Register getStackAddress(uint64_t Size, int64_t Offset,
    ...
    if (!SPReg)
      SPReg = MIRBuilder.buildCopy(p0, Register(MOS::SP)).getReg(0);
...
bool MOSCallLowering::lowerFormalArguments(
...
bool MOSCallLowering::lowerCall(MachineIRBuilder &MIRBuilder,
  ...
  if (!Info.OrigRet.Ty->isVoidTy()) {
    ...
    MOSIncomingValueHandler RetHandler(MIRBuilder, MRI, CC_MOS,
                                           MakeLive);
